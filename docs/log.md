...
- **`DEFAULT_DETAIL_FETCH_BUDGET` 400 → 1800.** Friday is ~691 events (operator-confirmed; the docs' earlier 271 was wrong), so no weekend day could finish on 400 — and a truncated day skips its deletion sweep, so cancellations silently stopped being detected. 1800 clears the largest day and stays under the 2000-subrequest ceiling (~1.04 subrequests/event with batched writes). Cron no longer overrides the budget; admin's "full" option now resolves to a budget that actually covers a day.
